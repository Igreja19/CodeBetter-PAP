...
<html><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <title>Your Projects - CodeBetter</title>
    <link rel="stylesheet" type="text/css" href="./css/font-awesome.css">
	<link rel="stylesheet" type="text/css" href="./css/projects.css">
	<script src="https://kit.fontawesome.com/56615f9db1.js" crossorigin="anonymous"></script>
...
<section class="projects">
	<header>
	    <a href="">
    		<!-- Logotipo é o background desse elemento -->
    		</div>
		</a>
...
<?php
			...
			$rowmembership = $plansql -> fetch(); ?>
			        ...
			        <div class="connection-type">
			            Connected To <span>CodeBetter</span>
			        </div>
	...
	<div class="frame project-manager">
		<div class="frameHeader">
			<h1>Your Projects</h1>
			<button class="addProj novo" title="Novo Projeto">
				<svg style="width:24px;height:24px" viewBox="0 0 24 24">
    			...
    			<button id="newproject" class="gitProj novo" style="color: white;background-color:<?php echo $row['color']; ?>">+

				<!-- The Modal -->
<div id="myModal" class="modal" style="cursor:default;">

<!-- Modal content -->
<div id="modal" name="modal" class="modal-content" style="border:50px;">
...
function updateLabel(input){                    
  ...
  <div id="createProject"> 	
	  ...
	  <center>
	  <table><tr><td>
	  <b><label style="color: #64696c;
	margin-top: 11px;
	margin-left: 2.6%;
	text-align: left;
	font-size: 17px;
	text-transform: uppercase;">CodeBetter:    </label></td><td><label id="label1" style="color: #64696c;
	margin-top: 11px;
	margin-left: 2.6%;
	text-align: left;
	font-size: 17px;
...
"></label></td></tr></table>
	  ...
	  <input type="text" id="projectname" name="projectname" onkeyup="updateLabel(this);"  placeholder="Project Name" style="width:75%;">
	  <br><br>
	  <input type="submit"  id="createprojbtn" value="Create Project" style="background-color:<?php echo $row['color'] ?>;cursor:pointer;color:white;display:inline-block;width:175px;"></form>
...
<div style="    position: relative;
	...
	background-color: #FFF;">
		...
		foreach ($files as $file) {
			$newfile = preg_replace('/[ -]+/' , '%20' ,$file); //substitui os espaços por %20 para reconhecer ao ir buscar o id

			$query2 = mysqli_query($conn, "SELECT * FROM community WHERE projectname='$file'");
			$row2 = mysqli_num_rows($query2);
			if($row2 >= '1') {
				echo "<a href='projects/".$_SESSION['username']."/".$file."'/>";
				echo "<div id='projectitem' class='projectitem'>$file <a href=delete.php?project=$newfile class='delete'><i class='fas fa-trash' style='float:right;margin-right:10px;'></i></a></div>"; 
				echo "</a>";
			} else {
			echo "<a href='projects/".$_SESSION['username']."/".$file."'/>";
			echo "<div id='projectitem' class='projectitem'>$file <a href=delete.php?project=$newfile class='delete'><i class='fas fa-trash' style='float:right;margin-right:10px;'></i></a><i class='fas fa-key' style='float:right;margin-right:10px; color:gold'></i></div>"; 
...
<?php if($row['membership'] == 4) { ?>
				<section class="free-acc-detected">
			<p>You can save only public projects, for private projects, consider making <a href="purchase.php">upgrade</a>.</p>
				</section>
<?php } else { ?>
					<section class="free-acc-detected">
			<p>You can now save public and private projects, enjoy your <a href="purchase.php">plan</a>.</p>
				</section>
<?php } ?>
				<button class="acess" id="send" disabled="">Select a Project...</button>
			<script>
				function delay (URL) {
    setTimeout( function() { window.location = URL }, 85 );
}
			</script>
				<a href="javascript:delay('community.php')"><button class="acess" id="send" style="background-color:<?php echo $row['color']; ?> !important;">Go to Community</button></a>
	</div>
